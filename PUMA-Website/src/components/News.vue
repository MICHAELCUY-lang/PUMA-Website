...
// Use the news API composable
const { articles, article, loading, error, fetchNews, fetchArticleById } = useNews();

const showModal = ref(false);
const currentArticle = ref<NewsItem | null>(null);

const openModal = async (item: NewsItem) => {
    // Fetch full article to increment views and ensure fresh content
    await fetchArticleById(item.id);
    currentArticle.value = (article as any).value || item;
                    ...
                    <div class="p-8 md:p-12">
                        ...
                        <div v-if="loading" class="flex items-center justify-center py-12">
                            <div class="text-center">
                                <div class="inline-block w-8 h-8 border-4 border-gray-300 rounded-full border-t-gray-800 animate-spin"></div>
                        ...
                        <div v-else class="space-y-8">
                            <article v-for="item in articles" :key="item.id"
                                ...
                                <div v-if="(item as any).featured_image" class="mb-4 overflow-hidden rounded-lg">
                                    <img :src="(item as any).featured_image || 'https://placehold.co/800x400?text=News'" alt="Featured image" class="object-cover w-full h-48 transition-transform duration-300 group-hover:scale-105" />
                                </div>
        ...
        <div v-if="showModal"
            ...
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" @click.stop>
                ...
                <div class="p-6 md:p-8">
                    ...
                    <div v-if="(currentArticle as any)?.featured_image" class="mb-6 overflow-hidden rounded-lg">
                        <img :src="(currentArticle as any).featured_image || 'https://placehold.co/1200x600?text=News'" alt="Featured image" class="object-cover w-full h-64" />
                    </div>
