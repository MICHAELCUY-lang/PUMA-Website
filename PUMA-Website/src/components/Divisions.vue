...
// Use the divisions API composable
const { divisions: apiDivisions, loading, error, fetchDivisions } = useDivisions()

// Map API divisions to component format
const divisions = computed(() => {
  return apiDivisions.value.map(div => ({
    name: div.code,
    title: div.title,
    image: div.image, // Image URL is already transformed in the composable
...
const visibleDivisions = computed(() => {
  ...
  return Array.from({ length: Math.min(5, divisions.value.length) }).map((_, i) => {
    ...
    <div class="container relative z-10 px-4 mx-auto">
      ...
      <div v-if="loading" class="flex items-center justify-center py-20">
        <div class="text-center">
          <div class="inline-block w-12 h-12 border-4 border-gray-300 rounded-full border-t-gray-800 animate-spin"></div>
      ...
      <!-- Divisions content -->
      <div v-else class="flex flex-col max-w-6xl gap-4 mx-auto md:gap-8 lg:flex-row">
