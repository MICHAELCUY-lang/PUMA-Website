...
const { cabinet, loading, error, fetchCabinetByName } = useCabinets()
...
const goToDivisionMembers = (divisionName: string) => {
    router.push(`/division/${divisionName}?cabinet=Kaustav`)
}
...
onMounted(async () => {
    ...
    try {
        // Fetch Kaustav cabinet with its divisions
        await fetchCabinetByName('Kaustav')
        ...
        console.error('Error loading Kaustav cabinet:', err)
    } finally {
        setTimeout(() => {
                ...
                <header class="relative mb-16 text-center">
                    <div class="relative inline-block">
                        ...
                        <h1 class="mb-4 font-mono text-5xl font-bold tracking-wider text-black uppercase transform sm:text-6xl -rotate-1">
                            Kaustav <span class="text-transparent bg-gradient-to-r from-gray-700 to-black bg-clip-text">Cabinet</span>
                        </h1>
                    </div>
