...
const user = reactive({
    ...
    linkedin: '',
    instagram: '',
    personalDescription: 'Update your personal description here.',
    batch: '2024'
});
...
onMounted(() => {
    ...
    if (authUser.value) {
        ...
        // Load member data if user is a member
        if (authUser.value.member) {
            user.role = 'Member';
            user.position = authUser.value.member.position;
            user.batch = authUser.value.member.batch;
            user.joined = new Date(authUser.value.member.joined_date).toLocaleDateString('en-US', { 
                year: 'numeric', 
                month: 'long', 
                day: 'numeric' 
            });
            user.cabinet = authUser.value.member.cabinet?.name || 'No Cabinet';
            user.division = authUser.value.member.division?.name || 'No Division';
        } else {
            user.role = 'Guest';
        }
        ...
        if (savedProfile) {
            try {
                ...
                // Only load editable fields from localStorage
                user.avatar = profile.avatar || user.avatar;
                user.linkedin = profile.linkedin || '';
                user.instagram = profile.instagram || '';
                user.personalDescription = profile.personalDescription || 'Update your personal description here.';
            } catch (e) {
...
const saveProfile = () => {
    // Update user data (except name and email which come from auth)
                    ...
                    <div class="relative px-8 pb-8">
                        ...
                        <div class="pt-24">
                            <div class="flex items-center justify-between mb-6">
                                <div class="flex-1">
                                    <h2 class="font-mono text-3xl font-bold text-gray-900">{{ user.name }}</h2>
                                </div>
