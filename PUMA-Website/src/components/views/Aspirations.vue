...
import { useAspirations } from '@/composables/useAspirations';
...
// Use the aspirations composable
const { aspirations, loading: isLoading, fetchAspirations, submitAspiration: submitAspirationAPI } = useAspirations();
...
const categoryToEvidenceType = (type: string): string => {
    const upperType = type.toUpperCase();
    return types.includes(upperType) ? upperType : 'GENERAL';
}
...
const submitAspiration = async () => {
    if (newAspiration.value.name && newAspiration.value.content) {
        try {
            await submitAspirationAPI({
                name: newAspiration.value.name,
                content: newAspiration.value.content,
                type: newAspiration.value.type
            });
            
            newAspiration.value = { name: '', content: '', type: 'aspiration' };
                                ...
                                <div :class="`relative bg-gradient-to-br ${getColor(index)} border border-gray-200 shadow-lg hover:shadow-xl transition-all duration-300 group-hover:border-gray-400 group-hover:shadow-2xl`"
                                    ...
                                    <div class="p-6 pt-8">
                                        <div
                                            ...
                                            {{ categoryToEvidenceType(aspiration.type) }}
                                        </div>
                                        ...
                                        <div
                                            ...
                                            <div class="flex items-center">
                                                <span class="font-mono tracking-wider uppercase">{{ aspiration.status }}</span>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <form @submit.prevent="submitAspiration" class="space-y-6">
                        ...
                        <div>
                            <label
                                class="block mb-1.5 font-mono text-xs font-medium tracking-wider text-gray-600 uppercase">Your
                                Aspiration</label>
                            <textarea v-model="newAspiration.content" rows="5"
                        ...
                        <div>
                            <label
                                class="block mb-1.5 font-mono text-xs font-medium tracking-wider text-gray-600 uppercase">Type
                            </label>
                            <select v-model="newAspiration.type"
