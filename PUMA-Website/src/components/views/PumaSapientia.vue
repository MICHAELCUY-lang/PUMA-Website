...
// Use the cabinets composable
const { cabinet, fetchCabinetByName } = useCabinets()
...
const goToDivisionMembers = (divisionName: string) => {
    router.push(`/division/${divisionName}?cabinet=Sapientia`)
}
...
onMounted(async () => {
    ...
    try {
        // Fetch Sapientia cabinet with its divisions
        await fetchCabinetByName('Sapientia')
        
        if (cabinet.value && cabinet.value.divisions) {
            divisions.value = cabinet.value.divisions.map(div => ({
                id: div.id,
                code: div.code,
                name: div.code, // Use code as name to match the original structure (BOD, RNT, etc.)
                title: div.title,
                image: div.image || '',
        ...
        console.error('Error loading Sapientia cabinet:', err)
    } finally {
        setTimeout(() => {
        ...
        <section class="relative z-10 py-16">
            <div class="container px-4 mx-auto max-w-7xl">
                <header class="relative mb-16 text-center">
                    <div class="relative inline-block">
                        ...
                        <h1 class="mb-4 font-mono text-5xl font-bold tracking-wider text-black uppercase transform sm:text-6xl -rotate-1">
                            Sapientia <span class="text-transparent bg-gradient-to-r from-gray-700 to-black bg-clip-text">Cabinet</span>
                        </h1>
                    </div>
                ...
                <div v-else class="relative space-y-12">
                    ...
                    <div class="relative z-10 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 place-items-center">
